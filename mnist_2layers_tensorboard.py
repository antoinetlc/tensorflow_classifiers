...
def mnist_inferenceGraph(images, dimension, hiddenUnits1, hiddenUnits2, numberClasses):
    ...
    with tf.name_scope("hiddenLayer1"):
        ...
        tf.summary.histogram("weights_HL1", weights)
        tf.summary.histogram("biases_HL1", biases)
...
def main():
    ...
    tf.summary.scalar("Accuracy", testAccuracy)
    ...
    #Add a FileWriter for tensorboard
    ...
    for i in range(TRAINING_STEPS):
        ...
        if i%100 == 0:
            print(i)
            summary, currentAccuracy = sess.run([merged, testAccuracy], {xData: mnist.test.images, yData:mnist.test.labels})
